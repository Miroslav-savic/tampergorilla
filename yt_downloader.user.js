...
(function () {
  ...
  // ── Find the insertion point below the player ────────────────────────────
  function findInsertTarget() {
    // Below the video title / above description
    return document.querySelector('#above-the-fold')
      || document.querySelector('#title.ytd-watch-metadata')?.closest('ytd-watch-metadata')
  ...
  function mount() {
    ...
    if (!pr && tries < MAX_TRIES) {
      setTimeout(mount, RETRY_MS);
      return;
    }
    ...
    if (formats.length === 0) {
      if (tries < MAX_TRIES) { setTimeout(mount, RETRY_MS); return; }
      // No downloadable formats found (e.g., cipher-protected or live stream)
      console.warn('[YT Downloader] No direct download URLs found for this video.');
      return;
    }

    const target = findInsertTarget();
    if (!target && tries < MAX_TRIES) { setTimeout(mount, RETRY_MS); return; }
    if (!target) return;

    injectStyle();
    const title = getTitle(pr);
    const panel = buildPanel(formats, title);
